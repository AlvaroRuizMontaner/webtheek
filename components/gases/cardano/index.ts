...
export function croot(coef: number[]): number | string {
    ...
    /* ---------- Δ  < 0  (tres raíces reales) ---------------------- */
        ...
        const theta = Math.acos(Math.max(-1, Math.min(1, arg)));  // clamp de robustez que evita redondeos raros
    ...
    /* ---------- |Δ| ≤ eps  (raíces múltiples) --------------------- */
        ...
        return Math.max(zSimple, zDouble) - (b / (3 * a));
    } 
}